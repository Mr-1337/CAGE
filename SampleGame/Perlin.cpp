...
namespace Perlin
{
	...
	static float gradDot(int hash, float x, float y)
	{
	...
	float Perlin(float x, float y)
	{
		...
		int ba = permutation[permutation[xi] + yi + 1];
		int bb = permutation[permutation[xi + 1] + yi + 1];

		float x1 = lerp(gradDot(aa, fx, fy), gradDot(bb, 1.f - fx, fy), u);
		float x2 = lerp(gradDot(ab, fx, fy), gradDot(ba, 1.f - fx, fy), u);
